...
def main(args):
    ...
    with torch.profiler.profile(
        ...
        for _ in pbar:
            ...
            with torch.autograd.profiler.record_function("vae_decode"):
                with autocast("cuda",enabled = True):
                    samples = vae.decode(samples / 0.18215).sample

            with torch.autograd.profiler.record_function("all_reduce"):
            ...
            prof.step()  # 记录每个迭代的性能数据
        end_time = time.time()

    prof.export_stacks(os.path.join(log_dir, "stacks.txt"), "self_cuda_time_total")
